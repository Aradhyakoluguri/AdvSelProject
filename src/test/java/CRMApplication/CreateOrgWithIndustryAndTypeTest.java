...
public class CreateOrgWithIndustryAndTypeTest {
	@Test
	public void createorgwithindustryandtype() throws InterruptedException {
		...
		driver.findElement(By.name("accountname")).sendKeys("phoneix");
		...
		if(verifyorg.getText().contains("phoneix")) {
		...
		if(verifyind.getText().contains("Energy")) {
		...
		//verify type
				...
				if(verifytype.getText().contains("Customer")) {
		...
		driver.findElement(By.xpath("//a[text()='phoneix']/../..//a[text()='del']")).click();
		Thread.sleep(2000);
		//handle alert popup
